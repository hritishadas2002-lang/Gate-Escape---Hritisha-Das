...
def inject_css():
    st.markdown(
        ...
        h1, h2, h3, h4 {{ color: {TOREA_BAY}; font-family: 'Segoe UI', sans-serif; }}
        p, li, span, label {{ color: {COCOA}; }}
...
def load_data(path="sample_data.csv"):
    ...
    # (These raw columns retain casing noise by design; we standardise display-side
    #  only, without touching the underlying file.)
    def title_clean(series):
    ...
    df["travel_party_clean"] = title_clean(df["travel_party_type"]).map(party_map).fillna(
        title_clean(df["travel_party_type"]))

    visa_map = {
        "Visa-Free/On-Arrival": "Visa-Free/On-Arrival",
        "Transit-Visa-Required": "Transit-Visa-Required",
        "Not-Eligible": "Not-Eligible",
    }
    df["visa_bucket_clean"] = df["visa_bucket"].astype(str).str.strip().apply(
        lambda v: visa_map.get(v.title().replace("/On-Arrival".title(), "/On-Arrival"), None)
        or _normalise_visa(v))

    # Ensure boolean-ish flag columns are real booleans
    for col in ["conversion_flag", "package_viewed_flag", "completed_flag",
                "high_value_lead_flag", "safe_to_leave_flag"]:
...
with tabs[1]:
    ...
    # Build cumulative funnel: each stage counts passengers who reached AT LEAST that stage.
    n = len(df)
    reached_elig = df["eligibility_checked_clean"].astype(str).isin(["True"]).sum()
    reached_view = df["package_viewed_flag"].sum()
    reached_book = df["conversion_flag"].sum()
    reached_done = df["completed_flag"].sum()

    stage_vals = [n, int(reached_elig), int(reached_view), int(reached_book), int(reached_done)]
...
with tabs[3]:
    ...
    with c3:
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
        stl = df.groupby("safe_to_leave_flag")["conversion_flag"].mean() * 100
        stl.index = ["Not Safe to Leave", "Safe to Leave"] if False in stl.index else stl.index
        fig = px.bar(
            x=[str(i) for i in stl.index], y=stl.values,
            title="Conversion Rate by Safe-to-Leave Flag (%)",
            labels={"x": "Safe to Leave", "y": "Conversion %"},
            color=[str(i) for i in stl.index], color_discrete_sequence=[SHILO, TOREA_BAY],
        )
        fig.update_layout(showlegend=False)
        st.plotly_chart(style_fig(fig), use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------------------
# TAB 6 — DATA QUALITY & FEATURE ENGINEERING
# ---------------------------------------------------------------------
with tabs[5]:
    section_tag("DATA PREPARATION — CLEANING, TRANSFORMATION & FEATURE ENGINEERING")
    st.subheader("Data Quality and Feature Engineering")
    ...
    st.markdown(
        ...
        leaving the original raw fields intact as an audit trail.
        """
    )
